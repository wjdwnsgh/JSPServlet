...
public class CookieManager {
	...
	// 쿠키생성 : 생성시 response내장객체가 있어야 하므로 매개변수로 받아야함.
	public static void makeCookie(HttpServletResponse response, String cName,
			String cValue, int cTime) {
		Cookie cookie = new Cookie(cName, cValue); // 생성자를 통해 쿠키생성(쿠키명, 쿠키값)
		cookie.setPath("/");					   // 경로 설정
		cookie.setMaxAge(cTime);				   // 유효기간
		response.addCookie(cookie);      	 	   // 응답헤더에 추가하여 전송
	}
	
	// 쿠키값 읽기 : request 내장객체가 필요하므로 매개변수로 받아야함.
	...
	public static void deleteCookie(HttpServletResponse response, String cName) {
		/*
			쿠키는 삭제를 위한 메서드가 별도로 없고, 빈값과 유효기간이 0으로 설정하면
